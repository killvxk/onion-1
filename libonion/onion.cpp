...
DWORD EmbedEncryptedResource(wchar_t* pwszVehicleFile, wchar_t* pwszPayloadFile, wchar_t* pwszResourceType, wchar_t* pwszResourceName, PBYTE pEncKey, DWORD dwEncKeyLen) {
	...
	do {
		...
		// Allocate buffer to store raw file bytes
		pPayloadData = (BYTE*)malloc(dwEncKeyLen + cbPayloadFileSize);
		...
		memcpy(pPayloadData, pEncKey, dwEncKeyLen);

		// Read raw file bytes into memory
		if (ReadFile(hPayloadFile, (pPayloadData + dwEncKeyLen), cbPayloadFileSize, &cbPayloadBytesRead, NULL) == FALSE) {
			TRACE(L"ReadFile error: %d", GetLastError());
			break;
		}

		// In-place encrypt the raw file bytes
		XORCrypt((pPayloadData + dwEncKeyLen), cbPayloadBytesRead, pPayloadData, dwEncKeyLen);
		...
		// Update the resource
		if (UpdateResource(hResource, pwszResourceType, pwszResourceName, MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), pPayloadData, (dwEncKeyLen + cbPayloadBytesRead)) == FALSE) {
...
DWORD ExtractEncryptedResource(wchar_t* pwszVehicleFile, wchar_t* pwszPayloadFile, wchar_t* pwszResourceType, wchar_t* pwszResourceName, PBYTE pEncKey, DWORD dwEncKeyLen) {
	...
	do {
		...
		pPayloadDataCopy = (PBYTE)malloc(dwPayloadFileSize - dwEncKeyLen);
		if (pPayloadDataCopy == NULL) {
			TRACE(L"malloc error: %d", GetLastError());
			break;
		}
		memcpy(pPayloadDataCopy, (pPayloadData + dwEncKeyLen), (dwPayloadFileSize - dwEncKeyLen));

		// Decrypt the bytes
		XORCrypt(pPayloadDataCopy, (dwPayloadFileSize - dwEncKeyLen), pPayloadData, dwEncKeyLen);
		...
		if (WriteFile(hPayloadFile, pPayloadDataCopy, (dwPayloadFileSize - dwEncKeyLen), &dwPayloadFileBytesWritten, FALSE) == FALSE) {
